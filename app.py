...
import random
import os
import requests  # Added for network connections
...
if 'remote_data_url' not in st.session_state:  # New for remote data
    st.session_state.remote_data_url = ""
if 'use_remote_data' not in st.session_state:  # New for remote data
...
# Fetch remote data with error handling
...
# Load data from either local or remote source
def load_data_source():
    if st.session_state.use_remote_data and st.session_state.remote_data_url:
        ...
        if os.path.exists(DATA_FILE):
            ...
            st.error("Local data file not found")
            return None
...
# Enhanced keyword search with filters (unchanged)
...
# Filter results based on multiple criteria (unchanged)
...
def show_settings():
    ...
    with st.form("network_settings"):
        ...
        use_remote = st.checkbox(
            ...
            help="Enable to load data from a remote URL instead of local file"
        )
        ...
        remote_url = st.text_input(
            ...
            help="URL to a JSON data file with the same structure as the local file"
        )
...
# Home page layout with added settings button
def show_home():
...
# Main app flow with added settings page
def main():
    st.set_page_config(
